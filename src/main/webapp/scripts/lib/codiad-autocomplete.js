...
(function (global, $) {
    ...
    codiad.autocomplete = {
        ...
        _computeTopOffset: function () {
            ...
            if (cursor.length > 0) {
                var fontSize = this._getEditor().container.style.fontSize.replace('px', '');
                var interLine = 1.7;
                cursor = $(cursor[0]);
        ...
        _getEditSession: function () {
            return this._getEditor().getSession();
        },

        _getDocument: function () {
            return this._getEditSession().getDocument();
        },
